...
theorem theorem6_density (b : ℕ) (hb : 2 ≤ b) :
  ∃ (d1 d2 : ℝ), ∀ (x : ℕ), x ≥ b ^ b →
    0 < d1 ∧ 0 < d2 ∧
...
theorem theorem6_lower_bound (b : ℕ) (hb : 2 ≤ b) :
  (∃ C₁ B : ℝ,
    0 < C₁ ∧
    (∀ (s : ℕ), (s ≥ B) →
...
theorem theorem6_upper_bound (b : ℕ) (hb : 2 ≤ b) :
  (∃ C₂ B : ℝ,
      0 < C₂ ∧
      (∀ (s : ℕ), (s ≥ B) →
      let rs := Real.rpow s ((b : ℝ) / (b - 1))
...
theorem theorem6_upper_bound_neg (b : ℕ) (hb : 2 ≤ b) :
